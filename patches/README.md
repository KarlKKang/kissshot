...
[Unraid 7.0](https://docs.unraid.net/unraid-os/release-notes/7.0.0/#excessive-flash-drive-activity-slows-the-system-down) introduces the `fastusr` feature, so that `/usr` is always run in RAM to avoid the performance issues. However, `/lib` is still mounted from the flash. 
...
- The `bzimage` file also contains a compressed and an uncompressed section:
  - The first uncompressed section is a small stub that does some basic hardware initialization and decompresses the second section. There's basically no way to verify the integrity of this section. We'll just assume that any failure in this stage should be catastrophic, like a firmware failure. This is the same reason why we don't bother checking the bootloader itself.
  - The second section is the compressed kernel image. By default, the kernel uses LZMA compression. LZMA is not checksummed. This patch recompiles the kernel with XZ compression, which is checksummed with CRC32. An error should be raised during boot if there's a checksum mismatch.
...
Therefore, this patch should have already achieved a decent level of integrity check for the Unraid system files ~~while keeping the changes minimal~~. The next-level integrity check should not be about using a different filesystem for the boot partition, but to use secure boot. This way, both the bootloader and the kernel will be verified. But of course secure boot brings its own complications and requires significant changes. I do not have a plan to look into this yet.
